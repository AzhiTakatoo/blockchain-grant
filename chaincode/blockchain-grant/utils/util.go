...
func StipendGarde(sortList []lib.ProofMaterial) []lib.SortStipend {
	var gardeList []lib.SortStipend
	var grade string
	var money string
	FirstClass := int(math.Ceil((0.03 * float64(len(sortList)))))
	SecondClass := int(math.Ceil((0.1 * float64(len(sortList)))))
	ThirdClass := int(math.Ceil((0.2 * float64(len(sortList)))))
	for i, val := range sortList {
		ii := i + 1
		if ii <= FirstClass {
			grade = "一等助学金"
			money = "4000"
		} else if ii <= FirstClass+SecondClass {
			grade = "二等助学金"
			money = "3000"
		} else if ii <= FirstClass+SecondClass+ThirdClass {
			grade = "三等助学金"
			money = "2000"
		} else {
			grade = "无助学金"
		}
		stipendRanking := lib.SortStipend{
			StipendId:    val.StipendId,
			StipendScore: val.StipendScore,
			Ranking:      int64(ii),
			Grade:        grade,
			Money:        money,
		}
		gardeList = append(gardeList, stipendRanking)
	}
	return gardeList
}
